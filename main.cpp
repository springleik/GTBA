...
int main(int argc, const char * argv[])
{
  ...
  cout << "Gated Toneburst Analysis Program from <www.williamsonic.com>," __DATE__ << '.' << endl;
  if (1 == argc)
  {
  ...
  while ((s = *++argv))
  {
    ...
    // command selector
    if      (!strncmp("aiff", s, 3))     {theFileType = typeAIFF;}
    else if (!strncmp("amplitude", s, 3)){setAmplitude(*++argv);}
    else if (!strncmp("average", s, 3))  {setAveraging(*++argv);}
    else if (!strncmp("cos1", s, 4))     {theBurstType = simpleCosine;}
    else if (!strncmp("cos2", s, 4))     {theBurstType = harmonicCosine;}
    else if (!strncmp("count", s, 3))    {setNumBursts(*++argv);}
    else if (!strncmp("cycles", s, 3))   {setNumCycles(*++argv);}
    else if (!strncmp("delay", s, 3))    {setDelay(*++argv);}
    else if (!strncmp("help", s, 3))     {setHelp();}
    else if (!strncmp("interval", s, 3)) {setBurstInt(*++argv);}
    else if (!strncmp("left", s, 3))     {setLeftAmpl(*++argv);}
    else if (!strncmp("minimum", s, 3))  {setBurstMin(*++argv);}
    else if (!strncmp("phase", s, 3))    {phase = ('+' == c);}
    else if (!strncmp("pilot", s, 3))    {pilot = ('+' == c);}
    else if (!strncmp("read", s, 3))     {readInputFile(*++argv);}
    else if (!strncmp("right", s, 3))    {setRightAmpl(*++argv);}
    else if (!strncmp("show", s, 3))     {reset(); showDetails(cout);}
    else if (!strncmp("sin1", s, 4))     {theBurstType = simpleSine;}
    else if (!strncmp("sin2", s, 4))     {theBurstType = harmonicSine;}
    else if (!strncmp("start", s, 3))    {setStartFreq(*++argv);}
    else if (!strncmp("stop", s, 3))     {setStopFreq(*++argv);}
    else if (!strncmp("verbose", s, 3))  {verbose = ('+' == c);}
    else if (!strncmp("wave", s, 3))     {theFileType = typeWAVE;}
    else if (!strncmp("write", s, 3))    {writeOutputFile(*++argv);}
    else    {reportError(s);}
  }
  return 0;
}
...
// set methods
void setStartFreq(const char *s) {if (s && *s) startFreq = stopFreq = atof(s);}
void setStopFreq(const char *s)  {if (s && *s) stopFreq = atof(s);}
void setAveraging(const char *s) {if (s && *s) averaging = atoi(s);}
void setBurstInt(const char *s)  {if (s && *s) burstInterval = atoi(s);}
void setNumBursts(const char *s) {if (s && *s) numBursts = atoi(s);}
void setNumCycles(const char *s) {if (s && *s) numCycles = atoi(s);}
void setAmplitude(const char *s) {if (s && *s) leftAmpl = rightAmpl = atof(s);}
void setLeftAmpl(const char *s)  {if (s && *s) leftAmpl = atof(s);}
void setRightAmpl(const char *s) {if (s && *s) rightAmpl = atof(s);}
void setBurstMin(const char *s)  {if (s && *s) burstMin = atoi(s);}
void setDelay(const char *s)     {if (s && *s) delay = atoi(s);}
void reportError(const char *s)  {if (s && *s) cout << "Unknown command: " << s << endl;}
void setHelp(void)
{
  cout << "Arguments are minus sign '-' with (at least) the first four characters from keywords listed below."
  ...
  "\n -phase     (flip phase on output)"
  "\n -pilot     (inhibit pilot tone)"
  ...
  "\n -verbose   (inhibit verbose output)"
...
void readInputFile(const char *s)
{
  if (!s || !*s)
  {
...
bool readAIFFheader(ifstream &inFile)
{
...
bool readWAVEheader(ifstream &inFile)
{
...
void readInputData(ifstream &inFile)
{
...
void readInputBurst(ifstream &inFile)
{
...
void writeOutputFile(const char *s)
{
  if (!s || !*s)
  {
    cout << "Required file name missing." << endl;
...
void writeAIFFfile(ofstream &outFile)
{
...
void writeWAVEfile(ofstream &outFile)
{
...
void writeOutputData(ofstream &outfile)
{
	int i = 0, j = 0;	// local loop indices
	short a = 0, b = 0;
	double y = 0.0;
...
void writePilotTone(ofstream &outfile)
{
...
bool signDiff(int a, int b) {return (0 > (a ^ b));}

// scan sound data to find 441 Hz pilot tone
void findPilot(ifstream &inFile)
{
  ...
  int minVal[NChan] = {-1,-1,-1,-1}, maxVal[NChan] = { 1, 1, 1, 1};
  ...
  for (int i = 0; i < (10 * sampleRate); i++)
  {
    ...
    // write debug output
    bool first = true;
    if (outFile) for (int j = I1; j < NChan; j++)
    {
    ...
    {
      if (next[Q1] > maxVal[Q1]) {maxVal[Q1] = next[Q1]; maxPos[Q1] = i;}
      if (next[Q1] < minVal[Q1]) {minVal[Q1] = next[Q1]; minPos[Q1] = i;}
    }
    if (signDiff(prev[Q1], next[Q1]))
    {
      if (next[I1] > maxVal[I1]) {maxVal[I1] = next[I1]; maxPos[I1] = i;}
      if (next[I1] < minVal[I1]) {minVal[I1] = next[I1]; minPos[I1] = i;}
    }
    if (signDiff(prev[I2], next[I2]))
    {
      if (next[Q2] > maxVal[Q2]) {maxVal[Q2] = next[Q2]; maxPos[Q2] = i;}
      if (next[Q2] < minVal[Q2]) {minVal[Q2] = next[Q2]; minPos[Q2] = i;}
    }
    if (signDiff(prev[Q2], next[Q2]))
    {
      if (next[I2] > maxVal[I2]) {maxVal[I2] = next[I2]; maxPos[I2] = i;}
      if (next[I2] < minVal[I2]) {minVal[I2] = next[I2]; minPos[I2] = i;}
    }
  }
  ...
  if (verbose)
  {
    ...
    {
      cout << j << ',' << minVal[j] << ',' << minPos[j] << ',' << maxVal[j] << ',' << maxPos[j] << endl;
    }
  }
  ...
  delay = offset;
  if (verbose) {cout << "Pilot tone found at offset," << offset << ",in chan," << maxChan << endl;}
